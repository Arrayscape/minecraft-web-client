...
describe('resumableSocket', () => {
  ...
  it('reconnects and replays what the proxy has not confirmed', async () => {
    ...
    // The proxy confirmed nothing, so everything unconfirmed goes again —
    // including 'before', which it already has. That duplication is correct
    // here: only the proxy knows what actually arrived, and it discards below
    // its own receive count.
    expect(proxy.allReceived()).toBe('beforebeforeduring')
  })

  it('states what it has received in the reconnect URL', async () => {
    // The proxy replays the moment a transport attaches. If our true receive
    // count arrived after that, the replay would start from a stale
    // acknowledgement and resend bytes we already had — and a duplicate corrupts
    // a byte stream exactly as thoroughly as a gap, and just as silently.
    const socket = await connect()
    ...
    proxy.kill()
    await waitFor('a reconnect', () => proxy.urls.length === 2, 5000)

    expect(proxy.urls[0]).not.toContain('received=')
    expect(proxy.urls[1]).toContain('received=10')
  })
  ...
  it('replays only the unconfirmed remainder after an acknowledgement', async () => {
    ...
    proxy.kill()
    await waitFor('a resume', () => state(socket).resumes === 1, 5000)
    await waitFor('the replay', () => proxy.bytesReceived() === 14, 5000)

    // Everything once, then only the tail the proxy had not accounted for.
